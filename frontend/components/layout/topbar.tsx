...
import { Bell, Check, CheckCheck, Moon, Search, Settings, Sun, X } from "lucide-react";
...
export function Topbar() {
  ...
  const dismissedReminders = useUiStore((s) => s.dismissedReminders);
  const dismissReminders = useUiStore((s) => s.dismissReminders);
  ...
  const reminderKey = (r: { journal_id: number; due_milestone: number }) =>
    `${r.journal_id}:${r.due_milestone}`;

  const unreadAlerts = (alerts ?? []).filter((a) => !a.is_read);
  // 过滤掉已被本地忽略的复盘提醒（复盘提醒无后端已读态，用本地持久化）
  const visibleReminders = (reminders ?? []).filter(
    (r) => !dismissedReminders.includes(reminderKey(r)),
  );
  const reminderCount = visibleReminders.length;
  const unread = unreadAlerts.length + reminderCount;

  const markAllReadAll = () => {
    if (unreadAlerts.length > 0) markAllRead.mutate();
    if (visibleReminders.length > 0) dismissReminders(visibleReminders.map(reminderKey));
  };
    ...
    <header className="relative z-30 flex h-[60px] shrink-0 items-center gap-5 border-b border-border-subtle bg-surface px-6 card-shadow">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative" ref={bellRef}>
          ...
          {open && (
            <div className="absolute right-0 top-full z-50 mt-1 flex max-h-[460px] w-80 flex-col rounded-md border border-border-strong bg-elevated shadow-xl">
              {/* 头部：标题 + 全部已读 */}
              <div className="flex items-center justify-between border-b border-border-subtle px-3 py-2">
                ...
                {unread > 0 && (
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-caption text-tertiary hover:bg-base hover:text-primary"
                  >
                ...
                {unreadAlerts.length > 0 && (
                  <div className="px-1 py-1">
                    ...
                    {unreadAlerts.map((a) => (
                      <div
                        key={a.id}
                        className="group flex items-start gap-1 rounded-md px-2 py-2 hover:bg-base"
                      >
                        <button
                          onClick={() => {
                            setOpen(false);
                        ...
                        >
                          <span className="text-body text-primary">
                            {a.name} <span className="tnum text-tertiary">{a.symbol}</span>
                          </span>
                          <span
                            className={cn(
                              "text-caption",
                              a.alert_type === "TARGET" ? "text-up" : "text-down",
                            )}
                          >
                            {a.alert_type === "TARGET" ? t("alerts.target") : t("alerts.stop")}
                            {" · "}
                            <span className="tnum">
                              {t("alerts.threshold")} {a.threshold} / {t("alerts.triggered")}{" "}
                              {a.triggered_price}
                            </span>
                          </span>
                        </button>
                        <button
                          ...
                          className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded text-tertiary opacity-0 transition-opacity hover:bg-elevated hover:text-primary group-hover:opacity-100"
                        >
                    ...
                    {visibleReminders.slice(0, 8).map((r) => (
                      ...
                      >
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            dismissReminders([reminderKey(r)]);
                          }}
