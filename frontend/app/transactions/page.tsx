      ...
      <table className="w-full text-small">
        <thead>
          <tr className="border-y border-border-default label-caps">
            <th className="px-3 py-2 text-left font-normal">{t("tx.col.date")}</th>
            <th className="px-3 py-2 text-left font-normal">{t("tx.col.side")}</th>
            <th className="px-3 py-2 text-right font-normal">{t("tx.col.qty")}</th>
            <th className="px-3 py-2 text-right font-normal">{t("tx.col.price")}</th>
            <th className="px-3 py-2 text-right font-normal">{t("tx.col.fees")}</th>
            <th className="px-3 py-2 text-center font-normal">{t("tx.col.journal")}</th>
          </tr>
        </thead>
        <tbody>
          {isLoading ? (
            [0, 1, 2, 3, 4].map((i) => (
              <tr key={i} className="border-b border-border-subtle">
                ...
                <td className="px-3 py-2.5"><Skeleton className="h-4 w-14 ml-auto" /></td>
                <td className="px-3 py-2.5"><Skeleton className="mx-auto h-4 w-10" /></td>
              </tr>
            ))
          ) : !txs || txs.length === 0 ? (
            <tr>
            ...
            txs.map((tx, i) => {
              ...
              return (
                ...
                >
                  <td className="tnum px-3 py-2.5 text-primary">{formatDate(tx.trade_date)}</td>
                  <td className="px-3 py-2.5">
                    <span className={tx.type === "BUY" ? "text-up" : "text-down"}>
                      {tx.type === "BUY" ? t("tx.buy") : t("tx.sell")}
                    </span>
                  </td>
                  <td className="tnum px-3 py-2.5 text-right text-primary">
                    {formatQuantity(tx.quantity)}
                  </td>
                  <td className="tnum px-3 py-2.5 text-right text-primary">
                    {formatMoney(tx.price, tx.currency)}
                  </td>
                  <td className="tnum px-3 py-2.5 text-right text-secondary">
